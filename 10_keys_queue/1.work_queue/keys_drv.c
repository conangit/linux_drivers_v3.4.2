...
#define __DEBUG_KEYD_DRV_

#ifdef __DEBUG_KEYD_DRV_
...
struct key_dev {
    ...
    struct work_struct work_queue;          //工作队列
...
static void do_work_func(struct work_struct *work)
{
    ...
    dev = container_of(work, struct key_dev, work_queue);
...
static int keys_open(struct inode *inode, struct file *filp)
{
    ...
    INIT_WORK(&dev->work_queue, do_work_func);

    if (request_irq(dev->irq, key_handle_irq, IRQ_TYPE_EDGE_BOTH, dev->name, dev))
    {
        printk(KERN_ERR "Error:can't get assigned irq %d\n", dev->irq);
        goto fail;
    }

    debug_print("success request irq %d\n", dev->irq);
