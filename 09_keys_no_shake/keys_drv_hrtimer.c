...
static irqreturn_t key_handle_irq(int irq, void *dev_id)
{
    ...
    hrtimer_start(&dev->hrt, ns_to_ktime(1000 * 1000 * 20), HRTIMER_MODE_REL);     //20ms
    
    return IRQ_HANDLED;
}

static enum hrtimer_restart do_hrtimer_func(struct hrtimer *hrt)
{
    struct key_dev *dev;

    printk(KERN_DEBUG "enter function %s()\n", __FUNCTION__);

    dev = container_of(hrt, struct key_dev, hrt);
    ...
    // 将周而复始执行 而非按键按下中断执行
    // hrtimer_forward_now(hrt, ns_to_ktime(1000 * 1000 * 20));
...
//自旋锁
    ...
    dev->hrt.function = do_hrtimer_func;
