...
static void eeprom_write_byte(unsigned short addr, unsigned char xchar)
{
    //1.设置处理器为主模式+发送模式
    IICSTAT |= (3 << 6);
    ...
    //5.1.1.写入EEPROM内部地址高8位到IICDS,并清除挂起位
    IICDS = ((addr & 0xff00) >> 8);
    //6.1.1.清除中断
    i2c_clear_irq();
    //4.1.1.等待ACK 条件:ACK后产生中断
    i2c_ack();

    //5.1.2.写入EEPROM内部地址低8位到IICDS,并清除挂起位
    IICDS = (addr & 0xff);
    i2c_clear_irq();
    i2c_ack();
...
static void eeprom_random_read(unsigned short addr, unsigned char *buf, int size)
{
    int i;
...
void i2c_test(void)
{
    ...
    printf("\r\nInput at24c128_256_512 device addr, (ignore bit0):");
    scanf("%x", &dev_addr);
    printf("\r\nAt24c128_256_512 device addr:0x%02x\r\n", dev_addr);
    DEV_ADDR = dev_addr;

    while (1)
    {
        printf("\r\nInput eeprom write addr(short):");
        scanf("%x", &addr);
        printf("\r\nWrite addr:0x%04x\r\n", addr);
...
** 读写分析 
** 写入数据一次，将发生3次中断：
** 写入从器件地址，写入内部EEPROM地址，写入数据 
**
** 读N次数据，将发生 4+N*1次中断：
** 写入从器件地址，写入内部EEPROM地址 写入开始读取命令，虚拟读取一次，循环N次 
** 计3*N + N + 4 (24C02)
** 4*N + N + 5 (24C512)
** 总结：操作了iicds寄存器 则会产生相应中断
*/
